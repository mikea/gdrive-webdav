...
func (fs *fileSystem) Mkdir(_ context.Context, name string, perm os.FileMode) error {
	slog.Debug("Mkdir", slog.String("name", name), slog.Int("perm", int(perm)))
	...
	if err == nil {
		slog.Error("dir already exists", slog.String("name", name), slog.String("id", pID))
		return os.ErrExist
	}
...
func (fs *fileSystem) OpenFile(ctx context.Context, name string, flag int, perm os.FileMode) (webdav.File, error) {
	slog.Debug("OpenFile", slog.String("name", name), slog.Int("flag", flag), slog.Int("perm", int(perm)))
	name = normalizePath(name)
...
func (fs *fileSystem) RemoveAll(_ context.Context, name string) error {
	...
	if err != nil {
		slog.Error("error deleting file", slog.String("name", name), slog.String("error", err.Error()))
		return err
	}
...
func (fs *fileSystem) Rename(_ context.Context, oldName, newName string) error {
	slog.Debug("Rename", slog.String("old", oldName), slog.String("new", newName))

	newFileAndPath, err := fs.getFile(newName, false)
	if newFileAndPath != nil {
		slog.Error("file already exists", slog.String("name", newName))
		return os.ErrExist
	}
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	...
	if path.Dir(oldName) != path.Dir(newName) {
		slog.Error("dir change not implemented", slog.String("old", oldName), slog.String("new", newName))
	}
	...
	slog.Debug("Files.Update", slog.String("path", fileAndPath.path), slog.String("name", file.Name))
	u := fs.client.Files.Update(fileAndPath.file.Id, &file)
	_, err = u.Do()
	if err != nil {
		slog.Error("error updating file", slog.String("name", oldName), slog.String("error", err.Error()))
		return err
	}
...
func (fs *fileSystem) Stat(_ context.Context, name string) (os.FileInfo, error) {
	slog.Debug("Stat", slog.String("name", name))
	f, err := fs.getFile(name, false)

	if err != nil {
		slog.Error("error getting file", slog.String("name", name), slog.String("error", err.Error()))
		return nil, err
	}

	if f == nil {
		slog.Debug("Can't find file ", slog.String("name", name))
...
func (fs *fileSystem) getFile0(p string, onlyFolder bool) (*fileAndPath, error) {
	slog.Debug("getFile0", slog.String("path", p), slog.Bool("only_folder", onlyFolder))
	p = normalizePath(p)

	if p == "" {
		f, err := fs.client.Files.Get("root").Do()
		if err != nil {
			slog.Error("error getting root file", slog.String("error", err.Error()))
			return nil, err
		}
	...
	parentID, err := fs.getFileID(parent, true)
	if err != nil {
		slog.Error("can't locate parent", slog.String("parent", parent), slog.String("error", err.Error()))
		return nil, err
	}
	...
	slog.Debug("getfile0 query", slog.String("query", query))
	...
	slog.Debug("can not get file", slog.String("path", p))
	return nil, os.ErrNotExist
}
...
func (fs *fileSystem) readdir(file *drive.File) ([]os.FileInfo, error) {
	...
	slog.Debug("readdir query", slog.String("query", query))
	q.Q(query)
